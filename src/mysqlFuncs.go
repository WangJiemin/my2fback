...
func (this *TablesColumnsInfo) GetAllTableKeysInfoFromDb(db *sql.DB, dbTbs map[string][]string, batchCnt int) error {
	...
	for _, oneQuery := range querySqls {
		...
		for rows.Next() {
			...
			if !slice.ContainsString(dbTbKeysInfo[dbName][tbName][kName], colName) {
...
func GetSystemHomeNameAndAdderss() (hostname string, address string) {
	...
	if err != nil {
		GLogger.WriteToLogByFieldsErrorExtramsgExitCode(err, "fail to get system hostname", logging.ERROR, ehand.ERR_JSON_MARSHAL)
	} else {
		hostname = host
	}
	...
	if err != nil {
		GLogger.WriteToLogByFieldsErrorExtramsgExitCode(err, "fail to get system adderss", logging.ERROR, ehand.ERR_JSON_MARSHAL)
	}
